...
console.log('--- Step 1: Deleting Netztopf items from database ---');
...
const targetStrains = db.prepare(`
  ...
  FROM strains 
  WHERE LOWER(name) LIKE '%netztopf%' OR LOWER(breeder) LIKE '%netztopf%'
`).all();

console.log(`Found ${targetStrains.length} strains matching "Netztopf":`);
...
if (fs.existsSync(proposedJsonPath)) {
  try {
    ...
    const filtered = existing.filter(item => {
      ...
      return !name.includes('netztopf') && !breeder.includes('netztopf');
    });
