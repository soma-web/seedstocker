...
console.log('--- Step 1: Deleting Zusatzzahlung items from database ---');
...
const targetStrains = db.prepare(`
  ...
  FROM strains 
  WHERE LOWER(name) LIKE '%zusatzzahlung%' OR LOWER(breeder) LIKE '%zusatzzahlung%'
`).all();

console.log(`Found ${targetStrains.length} strains matching "zusatzzahlung":`);
...
if (fs.existsSync(proposedJsonPath)) {
  try {
    ...
    const filtered = existing.filter(item => {
      ...
      return !name.includes('zusatzzahlung') && !breeder.includes('zusatzzahlung');
    });
