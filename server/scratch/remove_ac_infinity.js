...
console.log('--- Step 1: Deleting AC Infinity items from database ---');

// Enable foreign keys
db.pragma('foreign_keys = ON');

const acStrains = db.prepare(`
  SELECT id, name, breeder 
  FROM strains 
  WHERE LOWER(name) LIKE '%ac infinity%' OR LOWER(breeder) LIKE '%ac infinity%'
`).all();

console.log(`Found ${acStrains.length} strains matching "AC Infinity":`);
console.table(acStrains);
...
if (fs.existsSync(proposedJsonPath)) {
  try {
    ...
    const filtered = existing.filter(item => {
      ...
      return !name.includes('ac infinity') && !breeder.includes('ac infinity');
    });
