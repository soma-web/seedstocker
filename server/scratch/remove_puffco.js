...
console.log('--- Step 1: Finding Puffco brand items ---');

// Enable foreign keys
db.pragma('foreign_keys = ON');

const puffcoStrains = db.prepare(`
  SELECT id, name, breeder 
  FROM strains 
  WHERE LOWER(name) LIKE '%puffco%' OR LOWER(breeder) LIKE '%puffco%'
`).all();

console.log(`Found ${puffcoStrains.length} strains matching "Puffco":`);
console.table(puffcoStrains);
...
if (fs.existsSync(proposedJsonPath)) {
  try {
    ...
    const filtered = existing.filter(item => {
      ...
      return !name.includes('puffco') && !breeder.includes('puffco');
    });
