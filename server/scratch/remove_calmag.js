...
console.log('--- Step 1: Deleting Calmag items from database ---');

// Enable foreign keys
db.pragma('foreign_keys = ON');

const calmagStrains = db.prepare(`
  SELECT id, name, breeder 
  FROM strains 
  WHERE LOWER(name) LIKE '%calmag%' OR LOWER(breeder) LIKE '%calmag%'
`).all();

console.log(`Found ${calmagStrains.length} strains matching "Calmag":`);
console.table(calmagStrains);
...
if (fs.existsSync(proposedJsonPath)) {
  try {
    ...
    const filtered = existing.filter(item => {
      ...
      return !name.includes('calmag') && !breeder.includes('calmag');
    });
