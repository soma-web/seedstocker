...
console.log('--- Step 1: Deleting breeder Headshop from database ---');

// Enable foreign keys
db.pragma('foreign_keys = ON');

const countBefore = db.prepare(`SELECT COUNT(*) as c FROM strains WHERE LOWER(breeder) = 'headshop'`).get().c;
console.log(`Found ${countBefore} strains with breeder 'Headshop'.`);

const res = db.prepare(`DELETE FROM strains WHERE LOWER(breeder) = 'headshop'`).run();
console.log(`Deleted ${res.changes} strain entries from database.`);

const countAfter = db.prepare(`SELECT COUNT(*) as c FROM strains WHERE LOWER(breeder) = 'headshop'`).get().c;
console.log(`Remaining Headshop strains in DB: ${countAfter}`);
...
if (fs.existsSync(proposedJsonPath)) {
  try {
    ...
    const filtered = existing.filter(item => item.breeder && item.breeder.toLowerCase() !== 'headshop');
    fs.writeFileSync(proposedJsonPath, JSON.stringify(filtered, null, 2));
    console.log(`Updated ${proposedJsonPath} (removed Headshop entries, ${filtered.length} entries remaining).`);
  } catch (err) {
