...
const rowsStrains = sqlite.prepare(`
  ...
  FROM strains
  WHERE LOWER(breeder) LIKE '%groß%' OR LOWER(breeder) LIKE '%gross%' OR LOWER(breeder) LIKE '%noname%'
  GROUP BY breeder
`).all();
...
const rowsEntries = sqlite.prepare(`
  ...
  WHERE LOWER(extracted_breeder) LIKE '%groß%' OR LOWER(extracted_breeder) LIKE '%gross%' OR LOWER(extracted_breeder) LIKE '%noname%'
